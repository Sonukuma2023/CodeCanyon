        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body bg-light">
                ...
                <form id="adminReplyForm">
                    @csrf
                    <div class="mb-3">
                        <label for="admin_reply" class="form-label fw-semibold">Your Reply</label>
                        <textarea name="admin_reply" id="admin_reply" rows="5" class="form-control shadow-sm" placeholder="Type your response...">{{ old('reply', $community->admin_reply) }}</textarea>
...
<script>
    $('#adminReplyForm').on('submit', function (e) {
        ...
        $.ajax({
			url: "{{ route('admin.replyCommunity', ['id' => $community->id]) }}",
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                admin_reply: $('#admin_reply').val()
            },
            ...
            error: function (xhr) {
				if (xhr.status === 422) {
					...
					if (errors.admin_reply) {
						$('#replyError').removeClass('d-none').text(errors.admin_reply[0]);
					}
				} else {
